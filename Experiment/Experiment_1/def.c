...
{
    //这里可以只收集错误信息，最后一次显示
    printf(FONT_COLOR_RED "Error type C :Grammar Error %s %s in Line %d\n" COLOR_NONE, msg1, msg2, line);
}
void prn_symbol()
{ //显示符号表
    int i = 0;
...
{ //对抽象语法树的先根遍历,按display的控制结构修改完成符号表管理和语义检查和TAC生成（语句部分）
    ...
    if (T)
    {
        ...
        case FOR:
            ...
            T->code = merge(5, genLabel(T->ptr[3]->Snext), T->ptr[0]->code, T->ptr[1]->code,
                            genLabel(T->ptr[1]->Etrue), T->ptr[2]->code, T->ptr[3]->code, genGoto(T->ptr[3]->Snext));
            break;
